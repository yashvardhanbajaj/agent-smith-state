...
| `ledger.csv` | One row per run: `ts,mode,value_usd,usdinr,wallet_usd,spx,ndx,smh,smh_asof,est_net_flows_usd,external_flow_usd,value_trust,notes`. Powers the book-value and vs-benchmark charts and the intraday refresher fast-path. Three columns added 2026-07-26: **`smh`** (the mandate's benchmark — last completed SMH close at or before the run, `smh_asof` records which); **`external_flow_usd`** (money in/out of the account — distinct from `est_net_flows_usd`, which only measures position-level buys/sells *inside* the book; until this is populated, cumulative performance vs SMH is not computable and the dashboard refuses to plot it); **`value_trust`** (`ok` / `suspect` / `stale_dated` — corrupt price-feed readings are excluded from peak, drawdown, and chart scales instead of silently poisoning them). |
...
| `dashboard.html` | **Generated, never hand-written** (since 2026-07-26) — built by `scripts/smith_dashboard.py` from the state files. Source for the "Portfolio Sweep — Agent Smith" artifact (stable path = stable artifact URL). The published URL is stored in `state.json` (`artifact_url`) so every session updates the same page instead of minting new links. |
| `narrative.json` | The only per-run *prose* input to the dashboard: `{"session_read", "macro"}`. Everything else the builder derives from state. Keeps narrative out of the generated HTML. |
