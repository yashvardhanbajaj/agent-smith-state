...
python3 scripts/smith_math.py triggers \
  ...
  --today 2026-09-01 > $SCRATCH/triggers_check.json 2>&1
if diff -q tests/golden/triggers_case1.json $SCRATCH/triggers_check.json > /dev/null; then
  echo "PASS: cmd_triggers output byte-identical to golden master"
else
  echo "FAIL: cmd_triggers output CHANGED -- diff below"
  diff tests/golden/triggers_case1.json $SCRATCH/triggers_check.json || true
  exit 1
fi
