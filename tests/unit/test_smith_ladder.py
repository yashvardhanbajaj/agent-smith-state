...
class TestClusterRotationRespectsClusterRoom:
    ...
    # Phase 2: the sell is 30% of the position's open risk ($900 of the $3,000 LAG, same as
    # legacy) and the buy is the $144 of risk that frees at the same 16% stop = $900. Same-cluster
    # credit-back still applies: the cluster is "full" (room $0) yet the swap must size its buy.
    def test_full_cluster_still_sizes_the_buy_leg(self):
        pair = self._pair({"Semis": {"cluster_room_usd": 0.0}})
        assert pair["sell_leg"]["suggested_size_usd"] == pytest.approx(900.0)
        assert pair["buy_leg"]["suggested_size_usd"] == pytest.approx(900.0)
...
class TestRestatementNoLongerRetires:
    """The restatement auto-retirement rule this class used to guard (added 2026-08-24, patched
    2026-09-08 for the paired-leg orphaning failure below) was REMOVED entirely on 2026-09-15 by
    user decision: repeat_count is not evidence a proposal stopped making sense, only evidence it
    wasn't acted on, and the user wants to judge that himself rather than have the desk delete the
    option out from under him. The paired-leg orphaning bug this class was built to catch
    (cluster_rotation-INTC-KLAC: BUY leg hit repeat_count 3 and auto-retired while the SELL leg
    stayed open, leaving a naked unfunded sell) can no longer happen for the same reason -- there
    is no more restatement rule to orphan a leg out from under its pair. These tests now confirm
    repeat_count is inert for retirement purposes, for both paired and unpaired proposals."""
    ...
    def _leg(self, **kw):
        pr = {"id": "P-246", "ticker": "KLAC", "action": "Buy KLAC", "status": "open",
              "direction_bucket": "BUY", "date": "2026-09-08", "repeat_count": 9,
              "trigger_type": "cluster_rotation", "pair_id": "cluster_rotation-INTC-KLAC",
              "pair_role": "buy"}
        pr.update(kw)
        return pr

    def test_a_paired_leg_with_a_high_repeat_count_stays_open(self):
        pr = self._leg()
        assert self._retire(pr) is None
        assert pr["status"] == "open"

    def test_an_unpaired_proposal_with_a_high_repeat_count_also_stays_open(self):
