...
|---|---|---|
| `none` | no ladder · older than `LADDER_TTL_DAYS` (14) · confidence low/unset · track record below coin-flip over ≥`LADDER_MIN_SCORED_CALLS` (6) scored calls **made since `ENGINE_EPOCH`** | nothing — the `rel_pp` rule runs exactly as before |
...
## Risk-sized sells, rotation buys and the materiality floor (Phase 2, added 2026-09-20)

**Why.** The engine's buys were sized in risk dollars (`smith_risk.stop_and_cap`); its sells were `market_value x constant` (0.20/0.25/0.30/0.40/1/3/0.50). That could not say "too small to matter": on 2026-09-20 it proposed rotating $56.37 of SKHY into CIEN on a $42.5K book, where a correct 10-point call earns $5.64. Sells now use the same currency as buys.

**Sizing.** `stop_pct = max(2 x atr20_pct, 3)`, `R_base = 0.5% x book` (~$212.71, the risk a full-size ENTRY carries). A sell's severity is a **fraction of the position's OWN open risk**, `R_open = mv x stop_pct/100`: `risk_removed = severity x R_open`, so `sell_size = min(severity x mv, mv)`. `smith_core.SEVERITY_R` holds the legacy magnitudes: overbought_distribution 0.25, catalyst_threat 0.20, scale_out_ladder 1/3 per rung, profit_rotation / cluster_rotation / cluster_bench_rotation / trend_breakdown 0.30, thesis_break 0.40, conviction_exit 0.50. **Why not R_base units:** a first version (0.5R/1.0R/2.0R) turned 20% catalyst trims into 100% liquidations on the 2026-09-20 run, because positions already sit near R_base. `trim_risk_cap` is computed in R_base units, `(R_open - R_base)/R_base`, so it trims exactly back to the ATR cap. `cluster_consolidation` stays a full exit. Parameters are published in `compute_triggers.json` -> `thresholds.sizing`; each row carries `severity_r`, `stop_pct`, `risk_removed_usd`, `sell_action`, `sizing_note` and `legacy_size_usd` (equal to `suggested_size_usd` for any un-clamped sell; a difference means exit-or-hold).

**Exit-or-hold.** A trim that would leave less than the minimum position (`smith_core.dust_usd`, $400 — the same dust number cmd_derisk and the lifecycle use) or removes more than 60% becomes a full exit, and `sizing_note` says so. A position already below the minimum is never partially trimmed (consolidation candidate).

**Rotation buys** are sized to the RISK the sell freed (`rotation_legs`), then capped by the buy's own conviction ticket (profit_rotation) and every existing clamp; `clamped_by` names the binding one and `rotation_risk` reports risk freed vs redeployed. A rotation is heat-flat only when neither the ticket nor a clamp binds — otherwise the difference goes to cash and a blocker says so. Every buy site now passes the real `cluster_room_usd` to `clamp_size`.

**Materiality.** A ticket must clear max($250, 0.40% of book, 0.20R/stop, 25x round-trip fee) (`policy.trade_materiality`, **confirmed:false** — the engine uses these but the user has not signed them; say so in the briefing). Sub-floor sells and rotation pairs are neither shrunk nor dropped: `vote: "below_materiality"` with `materiality_shortfall_usd`; full exits are exempt. Single-leg buys are annotated, not demoted. Only `vote:"live"` rows become proposals; report below_materiality rows as "too small to act on".

## Portfolio heat budget (Phase 3, added 2026-09-20)

**Why.** `policy.stop_loss_framework` records the 10% aggregate-open-risk cap BREACHED at 13.028% (08-26) and 11.947% (08-30) while sizing continued at the full per-position formula: the cap only ever printed a warning, and every ticket was sized as though it were alone in an empty book. Now every live buy of a run competes for ONE budget.

**The budget.** `H` = `aggregate_open_risk_usd` (the all-fire sum); `H_max` = cap% x total book; `rho` = `compute_correlation.stop_risk.avg_pairwise_correlation`; `H_eff_max = H_max x (0.70 + 0.30 x (1 - rho))`; `R_free = max(0, H_eff_max - H)`. Live 2026-09-20: H $2,743.98, H_max $4,254.19, rho 0.401 -> H_eff_max $3,742.42, R_free $998.44 (the plan's $3,692 used rho 0.44). A missing, degraded or stale (> 7d) correlation is treated as rho = 1.0 (budget = cap x 0.70) and `heat_budget.rho_basis` says so: **never a diversification credit that was not measured** (`take_the_credit: false`). `policy.heat_budget` is **confirmed:false** — say so in the briefing. The `correlation` pipeline stage now runs BEFORE `triggers` (it used to run after, so the file never existed in a fresh run).

**Allocation** (one post-pass in `cmd_triggers`, `_apply_heat_budget` -> `smith_ticket.allocate_heat`). Rotation PAIRS first: only the NET risk counts (`buy risk - risk its own sell frees`); net <= 0 always fits and raises the remaining room, net > 0 consumes exactly the net, a pair is never half-sized. SINGLE buys then go greedily by `smith_ticket.allocation_priority` (conviction_score today; Phase 4 swaps in EV per marginal risk — one line). A single that partly fits is sized to the room left (`clamped_by: "heat_room"`) only if it still clears materiality; otherwise it is **deferred**. Live, above-materiality standalone sells raise the budget (per ticker, never beyond that position's own open risk); a `below_materiality`, shadow or deferred sell never does.

**What you see.** Every buy row and rotation buy leg carries `risk_usd`, `book_heat_before_usd`, `book_heat_after_usd`, `heat_room_remaining_usd`, `heat_status`. A ticket that does not fit is emitted with `vote: "deferred"`, `deferred_by: "portfolio heat budget"`, `deferred_reason`, `would_be_size_usd` — NOT a proposal, but visible: report the queue ("3 ideas deferred by the heat budget"), never resize one up. `compute_triggers.heat_budget` holds the whole block (H, H_max, rho, basis, H_eff_max, R_free, sell credit, allocated, deferred, rooms).

**Over cap.** If `H > H_eff_max`, ZERO new net-risk buys: each deferred row says "book heat $X exceeds the correlation-adjusted budget $Y". Sells, exits and risk-neutral rotations still emit; no sell credit is taken on an over-cap book (a sell frees heat only when it executes).

**Named clamps in `clamp_size`** (only ever shrink a buy): `single_position` (max_single_position_pct, 12% of invested equity, exact cash-funded solve), `ai_capex` (max_ai_capex_factor_pct on its declared denominator; at 100% of invested equity a cash-funded buy cannot breach it, so it is honestly non-binding today), `cluster_risk_budget` (`H_eff_max x` the cluster's band ceiling, converted with the basis the drift table tested the ceiling on; `policy.heat_budget.cluster_sub_budget`), `heat_room`. `learned_stop` (measured 2.249x mid-tier ATR multiple) is now surfaced, advisory only, on mid-vol positions and buy rows; it changes no stop or size.

## Edge / EV gate and gate-and-scale feedback (Phase 4, added 2026-09-20)

**Why.** The scorecard had zero gating consumers: the only feedback path was the strategist LLM reading prose (not reproducible, auditable or testable), and no ticket carried any estimate of whether it was worth taking. `scripts/smith_edge.py` (pure) + `compute_edge.json` (written by the pipeline's `triggers` stage) close that. Order in `cmd_triggers`: candidates -> family verdict + EV gate (`_apply_edge_gate`) -> heat allocation -> materiality -> cash-above-band invariant.

**THE EVIDENCE RULE (user decision 2026-09-20: "earlier proposals were too broke").** Only proposals / shadow-journal rows dated on or after `smith_core.ENGINE_EPOCH` (2026-09-21) feed any verdict, n_eff, hit rate or p_win. Everything earlier came from the legacy engine (mechanical sizing, discarded stops, no materiality floor, no heat budget, no EV bar); its outcomes measure that engine, are inadmissible as calibration however large the sample, and stay REPORTED only as history: `scorecard.legacy` vs `scorecard.since_epoch` (n=0 until the first post-rebuild proposal is scored ~30 days on). `P_WIN_PRIOR = 0.45` is an UNCALIBRATED engineering prior chosen for conservatism, not measured from history; re-derive it once post-epoch ideas are scored. Signal-bucket hit rates in journal.json score SIGNALS against price and were never produced by the proposal engine, so the bucket penalty and unified `bucket_rate` reader are NOT epoch-filtered (a judgement call; overrule in `smith_edge.bucket_rate` if wanted).

**THE EVIDENCE RULE COVERS EVERY OUTCOME PATH, NOT ONLY THE SCORECARD (user decision 2026-09-21: "correct this and any similar older hit-rate issue").** Nothing dated before `ENGINE_EPOCH` may change a size, priority, vote, retirement or gate. Concretely: (1) `cmd_journal` still scores and LOCKS every journal entry (history intact) but tallies only firings on/after the epoch into `bucket_hit_rates`, `bucket_hit_rates_7d` and `name_bucket_grades`, requires n >= `BUCKET_RATE_MIN_N` (3) for a published rate, and stamps its output `bucket_rates_epoch`; `smith_edge.admissible_bucket_tables` is the ONE reader and treats a table without the current stamp as empty, so a stale persisted journal.json cannot leak legacy rates. (2) The Kelly conviction tilt (`track_record_multiplier`) is exactly 1.0 with the reason 'no post-epoch track record (since <epoch>) ... no tilt' until enough post-epoch signals mature; when it does apply, the reason names its window. (3) The priority penalty/reward (`bucket_adjustment`, `bucket_rate` -- which now labels `window: since <epoch>` and returns None below the floor) and the `signal_conviction` retirement read the same admitted tables; a `signal_conviction` proposal is retired on a falling POST-epoch rate only, and is kept open (cannot test) when there is none. (4) `ladder_authority` counts only scored calls whose `ladder_as_of` is on/after the epoch; with none, authority is exactly what zero scored calls gives (medium -> rank, high -> full, else none) -- it withdraws nothing and grants nothing, and the reasons say how many legacy calls were set aside. (5) `phase4.readiness` counts only post-epoch graded proposals (it reads 0 today) and reports `legacy_excluded`. (6) `trigger_journal.json`'s `hit_rate`/`hit_rate_by_key` are post-epoch only; the pre-epoch record is kept under `legacy_hit_rate*`. (7) Reporting: the weekly report, dashboard and resume summary lead with the current engine, label the legacy record 'legacy-engine history', and can no longer print the '<40% over n>=5' de-emphasis line from legacy data. Every one of these goes INERT until post-epoch outcomes mature (7d / 30d); that is deliberate. Left alone on purpose: `stops.atr_multiple.mid` (derived from the user's real stop-loss fills, advisory, never changes a size) and revealed-preference/dashboard-decision data (behavioural, not an outcome hit rate).

**Edge.** `p_win = clamp((w*family_hit + (1-w)*0.45) * (0.8+0.4*conviction/100), 0.15, 0.75)`, `w = n_eff/(n_eff+20)`; `payoff_R` from the analyst target HARD-CAPPED at `entry*(1+4R*stop%)` (a fresh `data_cache.analyst_targets` / journal target within 7 days, else payoff unknown -> shadow, never estimated); `fee_R = 0.30/stop%`; `EV_R = p*payoff - (1-p) - fee_R`.

**The gate applies to BUYS ONLY.** A buy (entry or rotation buy leg) with `EV_R < 0.15` (or no target, or a suppressed family, or an assessed rotation edge `EV_R(buy) - EV_R(holding the sell name) < 0.25`) is emitted `vote: "shadow"` with `refused_because` -- recorded, never silent, journaled to trigger_journal.json (with `direction_bucket`) so it is scored. **Sells are never EV-refused**: at the 0.45 prior a sell at a 1.0R payoff has EV_R -0.13 and would need p_win >= 0.59, which would refuse every sell including thesis_break / conviction_exit / catalyst_threat. Sells are governed only by the family verdict, and protective exits (thesis_break, conviction_exit, trim_risk_cap, factor_threat, catalyst_threat) ignore even a `suppressed` verdict. The held name's EV is defined only from its own conviction-derived p_win and capped target; if unmeasurable the rotation term is `unassessed` (neither passes nor refuses).

**Family verdicts** (per (direction, trigger_type) and per direction; HOLD excluded; n_eff = distinct `idea_key`s): `n_eff < 12` unproven x1.00 (never gates); CI straddles 0 watch x0.75; lb95 > 0 proven up to x1.25; `n_eff >= 20` and ub95 < 0 suppressed x0.00 (family -> shadow, keeps firing, keeps being scored, reversible). A thin family inherits its direction's verdict. Today every verdict is `unproven` (n_eff 0).

**Sizing.** Today's buy sizer already equals `R_base*f_conv*f_track*0.5` (conviction tier of the track-tilted score, stage fraction 0.5). Phase 4 ADDS `f_edge = clamp(EV_R/0.50, 0.25, 1.0)`, `f_family`, and a bounded `f_deemph`; conviction is not re-applied. Sells are gated, not scaled. Heat allocation now orders by EV_R.

**Safety invariant.** The gate may never take live BUY tickets to zero while cash is above band: the single highest-EV gate-refused buy that still clears materiality and heat is restored with `gate_override: "cash above band -- best available idea permitted"`; if none clears, `data_quality` says so.

**Other.** `deemphasize_buckets` (persisted to `state.strategist_deemphasis`) is a bounded -1 priority and x0.85 size, once, never zeroing; bucket hit-rate penalty (<45% over n>=8 -> -2) and `bucket_rate` (30d preferred, source labelled) unify the two readers; `score-shadow-journal` now runs in `postflight --phase commit` (deep/quick) so shadow families can earn their vote back; `_draft_scorecard_quote` leads with since-epoch expectancy then labelled legacy; the strategist slice carries `scorecard_gate` (the exact numbers the sizer used + the legacy-exclusion note).

## Trade ticket, statuses and retirement ownership (2026-09-21, rebuild Phase 5)

**The ticket.** Every BUY/SELL/TRIM proposal with a ticker written through `add-proposal --run-dir` gets `ticket_version: 1` and a nested `ticket` block (entry, stop, target, size, risk, edge, invalidation + machine-checkable `invalidation_checks`, horizon, gate, lots.ltcg_note, pair). It is built ONCE, by `smith_ticket.build_ticket`, from the run's own `compute_triggers.json` row (`draft-specs` builds the same ticket for the strategist's draft; `add-proposal` rebuilds it and compares). The legacy flat fields (`size_usd`, `stop_price_usd`, `risk_usd`, `risk_removed_usd`, `target_price_usd`, `expires_on`, `invalidation`, ...) keep their names and are WRITTEN FROM the ticket by `smith_ticket.apply_ticket` — they cannot diverge. **No backfill:** a row without `ticket_version` is LEGACY (the dashboard says so); no stop is ever synthesised onto history. A SELL/TRIM has no stop — its risk figure is `risk_removed_usd` and its `invalidation` says when it was wrong.

**Who owns what.** The strategist supplies `rationale`, `evidence_quality` and accept/reject. Size, stop, risk, horizon, invalidation are script-owned (`smith_core.TICKET_SCRIPT_OWNED_FIELDS`): a spec that disagrees is REJECTED under `--require-ticket` and corrected (with a data_quality line) otherwise. Rejected rather than "named override": an override is a second, unmeasured sizing engine, and the scorecard grades the ticket AS TAKEN. To disagree, drop the spec and say why in the notes.

**`--require-ticket` (DEFAULT OFF for one release).** Required — BUY: stop, size, risk, invalidation, horizon, rationale. SELL/TRIM: size, risk_removed, invalidation, horizon, rationale. HOLD and ticker-less cash proposals are exempt. Flag off: an incomplete spec is written legacy-shaped and a `data_quality` line lists what it lacked; an empty rationale is named, never silently defaulted. **To flip the default:** once one full run's `add-proposal` data_quality shows no "NOT a complete ticket" / "EMPTY rationale" lines, change `require_ticket = bool(getattr(args, "require_ticket", False))` in `cmd_add_proposal` to default True and add `--no-require-ticket`. A PAIRED_TRIGGERS proposal without a `<trigger>-<sell>-<buy>` `pair_id` is rejected in BOTH modes (P-352).

**Horizons** live in `smith_core.HORIZON_DAYS_BY_FAMILY` with a stated rationale per family; they are engineering assumptions until post-epoch outcomes exist. A family with no entry gets 7 days and the ticket says "DEFAULT".

**Statuses.** Vocabulary: `open | accepted_by_user | executed | auto_retired | superseded | dismissed_by_user | dismissed_by_desk`. `STATUS_ALIASES` translates OLD spellings on read (`fulfilled`/`filled` -> executed, `deferred`/`watch` -> open + `defer_until`); stored rows are never rewritten. Legacy `deferred`/`watch` rows are open for liveness (rechecked, retirable, clickable) but stay scoreable by stored spelling, and stay out of the legacy 7-day expiry (superseding them would shrink the scorecard and trip cmd_score's guard). **`executed` has ONE writer:** `smith_math.py reconcile-proposals` (also run at the top of `proposals` and in postflight): a live proposal becomes executed only where trades.json holds a fill with the same ticker and side, dated from the row's first statement to 10 days after its date, within 50% of its dollar size — never from acceptance alone. (The plan's symmetric +/-10 days was wrong: it matched brand-new tickets to fills made days BEFORE they existed.)

**Expiry.** A ticket's `expires_on` retires it (`ticket_expired`, `auto_retired`, accepted rows included — an acceptance is agreement then, not a standing order). The blunt 7-calendar-day expiry (`superseded`) survives ONLY for pre-ticket rows; the reentry 20-day `exited_on` clock likewise. Reviving a `ticket_expired` row is refused (it would be re-retired next run). `retires_when` for a ticket row is the ticket's `invalidation` — one author.

**Retirement ownership** (`smith_core.RETIREMENT_OWNERS`, tested): `cmd_proposals` auto-retires ONLY on mechanical ticker-level facts (trigger no longer fires, position gone, cap/cluster/cash cleared, expires_on passed, invalidation price/thesis met); `smith_validity` gives a verdict only for judgment/cross-source evidence (alpha against, your contradicting trade, strategist retire list, thesis conflict, earnings) and the user clicks. "Trigger no longer fires" alone is no longer a validity `weakened` reason (lifecycle owns it, including where it deliberately keeps a row open).
