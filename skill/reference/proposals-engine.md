...
**Sizing.** `stop_pct = max(2 x atr20_pct, 3)`, `R_base = 0.5% x book` (~$212.71). `size_usd = severity_r x R_base / (stop_pct/100)`, clamped to the position. `smith_core.SEVERITY_R` (units of R_base): overbought_distribution 0.5, catalyst_threat 0.5, scale_out_ladder 0.5 per rung, profit_rotation / cluster_rotation / cluster_bench_rotation / trend_breakdown 1.0, thesis_break 2.0, conviction_exit 2.0. `trim_risk_cap` is computed, `(R_open - R_base)/R_base`, so it trims exactly back to the ATR cap. `cluster_consolidation` stays a full exit. A 3%-ATR name gets a larger dollar trim than a 12%-ATR name for the same severity. The parameters are published in `compute_triggers.json` -> `thresholds.sizing`; each row carries `severity_r`, `stop_pct`, `risk_removed_usd`, `sell_action`, `sizing_note` and `legacy_size_usd` (the old number, kept one release).
