...
3. THESIS TABLE — for each holding: carry forward the prior thesis (write a fresh one-liner for new names or on first run — what is this position FOR?). Then verdict: INTACT / STRENGTHENING / BROKEN / WATCH, with one line of evidence (only developments after the watermark; "no new evidence — intact by default" is a valid line). A stock down on noise with an intact thesis is not a problem; flat price with deteriorating thesis is. NEWS BUDGET (both modes): fetch get_us_stocks_details news ONLY for names whose status is plausibly in question — prior WATCH/BROKEN statuses, new names, and big recent movers — and default the rest to "intact, no new evidence" (smith-signals already covers the full book's news; don't pay for it twice). In quick mode, list only names whose status CHANGED plus a one-line "rest unchanged" summary; in deep mode, show the full table but the news-fetch budget stays narrow — deep mode gets more analysis depth on the names in question, not more names scanned.
   ...
   **INSIDER CLUSTER FINDINGS (added 2026-09-07 — runs automatically alongside every monthly smith-quality dispatch, plus on-demand; see `reference/valuation-forensics.md`).** If the orchestrator's dispatch includes an `smith_edgar.py insider-cluster` finding for one of your tickers (a small inline JSON, not a slice ref), fold it in with the standard provenance tag: `insider_sell_into_rally` goes in `evidence_against` (e.g. `"3 insiders sold near the 52-week high (src: SEC Form 4, as_of 2026-09-07)"`), `insider_buy_the_drawdown` goes in `evidence_for`. Absence of a finding means it wasn't run this cycle (a non-quality-cadence run, or nothing to check), not that insiders are neutral — do not infer anything from silence here.
   ...
   **CHECK FOR PRECEDENT before writing a status CHANGE (added 2026-09-07).** If a name's verdict moves this run (any direction), run `python3 scripts/smith_math.py gaps --base-dir . --query "<2-4 word summary of the situation, e.g. 'measurement basis mismatch' or 'debut competitor listing'>"` before finalizing it. This is ranked lexical search over ~84 logged incidents — cheap, one call — not a database of stock calls, but this desk has repeatedly downgraded on a claim that turned out to be a measurement artifact (G58, the MU -51% HBM3E case) or a funding event mistaken for a technical one (CXMT's STAR listing). A relevant hit is a candidate precedent to weigh, not an override — cite its ID in your note if it changes how you'd otherwise have written the verdict; an empty result means nothing, not confirmation.
