...
**Step 0 — no cache to check.** This agent used to read a pre-open `rebound_prime.json` built by a heat-check self-schedule that was retired 2026-08-31 (last written 2026-07-17, never refreshed across 31 subsequent runs, no primer task ever created). Compute support levels live in step 6 — there is nothing to check first, and step 6 does not reference this cache either.
...
6. SUPPORT LEVELS — SCRIPT-COMPUTED, NO FETCH (rewritten 2026-09-15, closes a ~26-call/name efficiency gap). Each candidate in your embedded `compute_triggers.json` `rebound.candidates` block already carries `support_level`, `support_label` (`sma50`/`sma100`/`sma200`/`swing_low_60d`) and `support_distance_pct` — computed by `smith_marketdata.support_levels` from the run's own `bars.json` (the same 1y daily bars `indicators` already turned into ATR20/RSI14), the highest of SMA50/100/200 and a 60-session swing low that sits below last close. Read it from there; do not fetch it. FALLBACK ONLY, and only when a candidate's `support_level` is `null` (bars too short — recent IPO/spinoff, or the ticker fell out of `bars.json`'s universe): one single-symbol `get_stock_history` (daily, 1y) call for that ticker only, computed per section G's fallback rule. Never WebFetch Barchart for this any more, and never batch multi-symbol history (known_gaps G10 — silently auto-aggregates to weekly bars).
...
**G. Support level (script-computed, read not derived — rewritten 2026-09-15; feeds the `Support` column and each proposal's `support_usd`/`secondary_support_usd`).** For every ticker with an `action` of `rebuy`/`average`/`extend` (from E), read its candidate row's `support_level`/`support_label`/`support_distance_pct` (step 6):
- `support_usd` = `support_level`, less a 0.5% buffer.
- `secondary_support_usd`: if `support_label` is `sma50`, the next one down would be `sma100` or the swing low — you don't have that second rung from the embed, so set `secondary_support_usd` equal to `support_usd` and flag `"secondary approximate -- primary label was {support_label}"` in `data_quality`. This is a narrower fallback than before by design: the script gives you the ONE nearest floor, not a ladder.
- `support_level` is `null` (price sits below every SMA and the swing low, or bars too thin): `support_usd` = price − 2×ATR20 (from `atr20_pct` in your embed), tag the proposal's rationale "no support below price -- ATR floor only, broken trend", and treat it as a soft stay-out signal for rebuy purposes.
- Fallback-fetch case (step 6): support = higher of {20-day low, 50-day SMA} below price from the single-symbol history you pulled; secondary = 100-day low or 200-day SMA, whichever is lower. Flag `"<100d history for {T}, secondary approximate"` for recent IPOs/spinoffs.
No trendline-drawing, no chart-pattern judgment — pure arithmetic on a script-computed value, same "deterministic, not narrative" philosophy as the rest of this agent.
