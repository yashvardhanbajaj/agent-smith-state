...
9. PEER-RELATIVE STRENGTH (every mode) — maintain `peer_map` {TICKER:{"peer_etf":"","label":""}} in state.json, mapping each holding to a liquid sector-ETF proxy (seed on first run or for any newly-added ticker from smith-thesis's sector_map cluster: semi-memory/semicap/optical-interconnect → SMH; hyperscaler/compute → XLK; power-infra → XLU; anything defensive/non-AI-capex → XLP or XLV, whichever is the closer fit — use judgment, this is a proxy not a precise mapping).
   ...
   **CACHE-CHECK-FIRST for any ticker whose `peer_map` ETF is NOT "SMH" (added 2026-09-07, same pattern as task 1's analyst_targets check).** `compute_buckets.json`'s own `stale_peer_fallback_tickers` field already names exactly which of these tickers have a stale or missing `data_cache.rel_strength_1m_peer` entry — before this cache existed, this task recomputed the SAME BE/GEV/VRT-vs-XLU, MSFT/NBIS-vs-XLK override from a fresh fetch on every single dispatch, quick or deep, with no TTL gate at all. Read that field first: **only fetch peer-ETF/holding returns for the tickers it names.** Any non-SMH-peer ticker NOT in `stale_peer_fallback_tickers` already has a fresh cached reading — `cmd_buckets` used it directly (check that row's `peer_benchmark_used` field to confirm), and you do not need to touch it.

   For the tickers `stale_peer_fallback_tickers` actually names: ONE batched yfinance call for the DISTINCT peer ETFs they reference (typically 2–4 tickers, not the whole peer_map), covering 1-month return, plus the holdings' own 1-month returns if `get_us_stocks_details` doesn't already surface one. For each: `relative_strength_1m = holding_1m_return − peer_etf_1m_return`. This catches names quietly riding — or lagging — a sector-wide move that raw price-position (pos, day%) alone would miss (e.g. a name at pos=0.9 that's merely tracking a hot sector ETF, not genuinely leading it). Return these as `rel_strength_1m_peer_updates`: `{"TICKER": {"rel_pp": 9.24, "peer_etf": "XLU"}}` — this is a NEW cache, distinct from `rel_strength_1m_updates` below (which stays SMH-only, for the book-wide default). Return `peer_map` updates only for tickers whose mapping is new or changed — the orchestrator merges into its persistent copy.
...
{"signal_history":{"changed":{"TICKER":["bucket"]},"unchanged_count":0},
 ...
 "rel_strength_1m_peer_updates":{"TICKER":{"rel_pp":0,"peer_etf":""}},
 "data_quality":[]}
```
